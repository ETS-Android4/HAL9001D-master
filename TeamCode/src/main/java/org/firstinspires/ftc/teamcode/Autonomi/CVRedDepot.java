...
@Autonomous(name = "CVRedDepot", group = "compCV")
public class CVRedDepot extends BaseAutonomous {
    public @MainRobot
    Baguette robot;
...
*/
        ...
        boolean isBottomTier = Baguette.realPosition.getLevel() == 1;
        //scan
        HALTrajectory setForPlace = robot.mDrive.trajectoryBuilder(new Pose2d())
                .splineToConstantHeading(new Point2D(-30, 10 + ((isBottomTier) ? -1.5 : 0)), 0)
                .build();

        HALTrajectory forward = robot.mDrive.trajectoryBuilder(new Pose2d())
                .splineToConstantHeading(new Point2D(0, 10 + ((isBottomTier) ? + 1.5 : 0)), 0)
                .build();
        ...
        robot.telemetry.addData("vision: ", Baguette.biggest.area());
